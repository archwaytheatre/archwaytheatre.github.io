...
(defn re-sort [& things]
  (let [volunteering (shuffle (filter #(-> % first name (string/includes? "volunteer")) things))
        everything-else (interleave-all
                          ...
                          (filter #(-> % first name (string/includes? "other")) things))]
...
(core/page "getinvolved" "The Archway Theatre"
  ...
  [:div.content
   ...
   [:div.center
    [:a.hp1 {:href "javascript:toggle('volunteer')"} "Show / Hide Volunteering"]
    [:a.hp1 {:href "javascript:toggle('audition')"} "Show / Hide Auditions"]
    [:a.hp1 {:href "javascript:toggle('other')"} "Show / Hide Events"]
    ]

   [:br] [:br]

   [:div.getinvolved.volunteer {:data-sort "0"}
   ...
   (re-sort
     ...
     [:div.getinvolved.audition {:data-sort "0"}
    ...
    Playing dates 6th to 17th June 2023"]
     ...
     [:div.getinvolved.other
      ...
      [:div "Fortnightly singing, on Monday evenings 8pm - 9:30pm in the Function Room."]
      [:br]
      [:div "13th March, 27th March, 24th April, 8th May, 22nd May, 5th June"]
      [:br]
      [:div "£4 per session (£5 for non members)"]
   ...
   ; items - possible square image? text, possible call to action button / link
